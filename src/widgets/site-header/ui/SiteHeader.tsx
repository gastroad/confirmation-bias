...
export function SiteHeader({ user, signOut, back, title, hero = false }: SiteHeaderProps) {
  const wordmark = [hero ? styles.brand : styles.brandSmall, back && styles.wordmarkCompact]
    .filter(Boolean)
    .join(" ");
      ...
      <div className={styles.inner}>
        ...
        {title ? (
          <>
            ...
            <h1 className={styles.brandSmall}>{title}</h1>
          </>
        ) : (
          <Link href="/" className={styles.brandLink} aria-label={`${SITE_NAME} 홈`}>
            <Logo size={hero ? 28 : 20} className={styles.logo} />
            <h1 className={wordmark}>{SITE_NAME}</h1>
          </Link>
        )}
        {hero && <p className={styles.brandSub}>언론사 성향별 뉴스 보도 분석</p>}
