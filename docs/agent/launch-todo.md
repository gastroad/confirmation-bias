...
- [x] **파이프라인 실패 알림** ✅ GitHub 기본 알림으로 충족(2026-08-24)
      Actions 워크플로우 실패 시 GitHub이 저장소 소유자에게 메일을 보낸다. 별도 연동 불필요.
      (Slack 등이 필요해지면 그때 추가)
...
- [ ] **오래된 클러스터 아카이빙 / 정리**
      무한 누적 방지.
- [ ] **언론사 성향 분류 근거 투명성** — 부분 충족
      이용약관 3조에 "미디어 연구를 참고한 상대적 위치 표시이며 품질 평가가 아니다"라고
      명시했고 README에도 같은 취지를 적었다. **다만 어떤 연구를 참고했는지 출처가 없다.**
      17곳 각각의 분류 근거를 밝히는 문서가 있으면 신뢰도가 올라간다.
- [ ] **모니터링 / 분석**
